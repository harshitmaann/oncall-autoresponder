...
class SlackNotifier:
    ...
    def post_incident_brief(self, incident: Incident) -> None:
        text, blocks = self._format_blocks(incident)
        if not self.enabled:
            print("[slack] disabled (missing SLACK_BOT_TOKEN or SLACK_CHANNEL_ID)")
            print(text)
            return
        self.app.client.chat_postMessage(channel=self.channel, text=text, blocks=blocks)
    ...
    def _format_blocks(self, incident: Incident):
